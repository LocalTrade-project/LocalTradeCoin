...
#include "ui_zlocaltradecontroldialog.h"
...
std::list<std::string> ZLocalTradeControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZLocalTradeControlDialog::listMints;

ZLocalTradeControlDialog::ZLocalTradeControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZLocalTradeControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZLocalTradeControlDialog::~ZLocalTradeControlDialog()
{
    delete ui;
}

void ZLocalTradeControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZLocalTradeControlDialog::updateList()
{
...
void ZLocalTradeControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZLocalTradeControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZLocalTrade_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZLocalTradeControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZLocalTradeControlDialog::GetSelectedMints()
{
...
void ZLocalTradeControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
