...
void PrepareShutdown()
{
    ...
    RenameThread("localtrade-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "localtrade2.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "localtraded.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the LocalTrade and zLocalTrade money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in LocalTrade/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in LocalTrade/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, localtrade, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in LocalTrade/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all LocalTrade specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-backupzlocaltrade=<n>", strprintf(_("Enable automatic wallet backups triggered after each zLocalTrade minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizelocaltradeamount=<n>     " + strprintf(_("Keep N LocalTrade anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The Awesome LocalTrade Core Developers"), 2019)) + "\n" +
           "\n" +
...
{
    RenameThread("localtrade-loadblk");
...
/** Sanity checks
 *  Ensure that LocalTrade is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. LocalTrade Core is shutting down."));
...
#endif
    // Make sure only a single LocalTrade process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. LocalTrade Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("LocalTrade version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // LocalTrade: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZLocalTradeMinted();
                        RecalculateZLocalTradeSpent();
                    }
                    RecalculateLocalTradeSupply(1);
                }
                ...
                // LocalTrade: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of LocalTrade Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart LocalTrade Core to complete") << "\n";
        ...
        bool fEnableZLocalTradeBackups = GetBoolArg("-backupzlocaltrade", true);
        pwalletMain->setZLocalTradeAutoBackups(fEnableZLocalTradeBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeLocalTradeAmount = GetArg("-anonymizelocaltradeamount", 0);
//    if (nAnonymizeLocalTradeAmount > 999999) nAnonymizeLocalTradeAmount = 999999;
//    if (nAnonymizeLocalTradeAmount < 2) nAnonymizeLocalTradeAmount = 2;
    ...
    LogPrintf("Anonymize LocalTrade Amount %d\n", nAnonymizeLocalTradeAmount);
    ...
    /* Denominations
       ...
       For example:
       1LocalTrade+1000 == (.1LocalTrade+100)*10
       10LocalTrade+10000 == (1LocalTrade+1000)*10
    */
